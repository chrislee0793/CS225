    ...
    for(int i = 0; i < width; i++)
    {
        for(int j = 0; j < height; j++)
        {
            ...
            for(int k = layers.size() - 1; k >= 0; k--)
            {
            	...
            	if(found)
            		break;
            	int offsetW = coords[k].x;
        		int offsetH = coords[k].y;
            	if(i >= offsetW && i < (offsetW + layers[k].width()) && j >= offsetH && j < (offsetH + layers[k].height()))
            	{
            		*(*result)(i, j) = *(layers[k](i - offsetW, j - offsetH));
            		found = true;
            	}
            }
        }
