...
class BinaryTree
{
    ...
    private:
        ...
        void recursiveReverse(Node* target);

        bool recursiveOrdered(Node* target) const;

        void recursivePaths(string prefix, Node* top) const;

        int recursiveDistance(int length, Node* top) const;

        string TToString(T input) const;
