...
HuffmanTree::TreeNode * HuffmanTree::removeSmallest( 
	...
	if(singleQueue.size() == 0 || singleQueue.front() == NULL)
	{
		TreeNode* temp = mergeQueue.front();
		mergeQueue.pop();
		return temp;
	}
	if(mergeQueue.size() == 0 || mergeQueue.front() == NULL)
	{
...
void HuffmanTree::buildTree( const vector<Frequency> & frequencies ) {
	 ...
	 */
	 ...
	 {
	 	TreeNode* derp = new TreeNode(frequencies[i]);
	 	singleQueue.push(derp);
	 }
	 ...
	 while(singleQueue.size() + mergeQueue.size() != 1)
	 {
	 	...
	 	TreeNode* add = new TreeNode(temp1->freq.getFrequency() + temp2->freq.getFrequency());
	 	add->left = temp1;
	 	add->right = temp2;
	 	mergeQueue.push(add);
	 }
