...
int main() {
 ...
 /*  cout << "fullTree.pruneSize(0) = " << fullTree.pruneSize(0) << endl;
   ...
   //cout << "10" << fullTree.idealPrune(10) << endl;
   ...
   PNG testout(imgIn.width(), imgIn.height());
   
	for(int i = 0; i<imgIn.width(); i++)
		for(int j = 0; j<imgIn.height(); j++)
		{
			RGBAPixel b = fullTree2.getPixel(i, j);
			testout(i, j)->red = b.red;
			testout(i, j)->blue = b.blue;
			testout(i, j)->green = b.green;
		}

	testout.writeToFile("test.png");
