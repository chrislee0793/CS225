...
class KDTree
{
    public:
        ...
        int partition(vector< Point<Dim> > & points, int left, int right, int pivotIndex, int subpos);

        Point<Dim> select(vector< Point<Dim> > & list, int left, int right, int k, int subpos);

        void buildTree(vector< Point<Dim> > & newPoints, int axis);

        Point<Dim> median(vector< Point<Dim> > & newPoints, int axis);

        Point<Dim> findNearestNeighbor(int low, int high, int axis, const Point<Dim> & query) const;
